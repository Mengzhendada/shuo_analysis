    ...
    for(int i = 1;i<56;i++){
      ...
      if(!neg_H2.empty() && !pos_H2.empty()){
        ...
        for(auto it = neg_H2.begin();it!=neg_H2.end();++it){
          ...
          jout[(std::to_string(RunGroup)).c_str()]["neg"][(std::to_string(RunNumber)).c_str()]["target"] = "H2";
        ...
        for(auto it = pos_H2.begin();it!=pos_H2.end();++it){
          ...
          jout[(std::to_string(RunGroup)).c_str()]["pos"][(std::to_string(RunNumber)).c_str()]["target"] = "H2";
