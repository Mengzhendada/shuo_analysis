...
#include <string>
  ...
  void shms_rate_Dummy(){
    //void shms_rate_Dummy(int RunGroup){
    //  if(RunGroup == 0){
    ...
    for(int i = 1;i<56;i++){
      ...
      if(!neg_Dummy.empty() && !pos_Dummy.empty()){
        ...
        for(auto it = neg_Dummy.begin();it!=neg_Dummy.end();++it){
          ...
          jout[(std::to_string(RunGroup)).c_str()]["neg"][(std::to_string(RunNumber)).c_str()]["target"] = "Dummy";
        ...
        for(auto it = pos_Dummy.begin();it!=pos_Dummy.end();++it){
          ...
          jout[(std::to_string(RunGroup)).c_str()]["pos"][(std::to_string(RunNumber)).c_str()]["target"] = "Dummy";
