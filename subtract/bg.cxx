...
void bg(int RunGroup = 0){
  ...
  ////change bgs first bg selection, left and right bg peaks
   ...
   coin_bg_pos_after_1 = enough ? coin_peak_center_pos+2.5*4.008 : coin_peak_center_pos + 1.5*4.008; 
   coin_bg_pos_after_2 = enough ? coin_peak_center_pos+8.5*4.008 : coin_peak_center_pos + 7.5*4.008; 
  ...
  auto h_coin_dp = d_pos_pi_bg_1.Histo1D({"","",100,-10,22},"P.gtr.dp");
  h_coin_dp->GetXaxis()->SetTitle("shms dp");
  h_coin_dp->GetYaxis()->SetTitle("counts");
  //auto h_coin_dp = d_pos_pi_bg_1.Histo2D({"","",100,-10,22,100,0,100},"P.gtr.dp","CTime.ePiCoinTime_ROC2");
  TCanvas* c_coin_dp = new TCanvas();
  h_coin_dp->DrawCopy("hist");
  c_coin_dp->SaveAs("results/acc_subtract/check.pdf");
  auto h_dp_pimain = d_pos_pi_pimain.Histo1D({"","",100,-10,22},"P.gtr.dp");
  TCanvas* c_dp_main = new TCanvas();
  h_dp_pimain->DrawCopy("hist");
  c_dp_main->SaveAs("results/acc_subtract/check2.pdf");
