...
double fit_kaondecay(double* x, double* params) {
  ...
  // params[1] is the momentum, the mean peak is calculated by the momentum, need to be fixed to be
  // what it is params[2] is the sigma of the nodecay kaon, need to be determined by fitting
  double        all_kaon = 0;
  ...
  while (getline(infile, line)) {
    ...
    pa[0] =
        ...
        (2 * 0.8235); // the amplitude of the pion(from kaon decay) peak portion of nodecay kaons.
    pa[1]                   = params[3];
    pa[2]                   = 0.2;
    pa[3]                   = meter;
    pa[4]                   = params[0];
...
public:
  ...
  double DoEval (const double *x) const {
    ...
    std::cout <<  "sigma_pi   = " << sigma_pi << "\n";
    std::cout <<  "A_K        = " << A_K      << "\n";
    std::cout <<  "sigma_K    = " << sigma_K  << "\n";
...
void SHMS_rf_twocomplicatedfit_high(int RunGroup = 0) {
  ...
  if (!neg_D2.empty() && !pos_D2.empty()) {
    ...
    for (auto it = std::next(delta_cut_num.begin()); it < delta_cut_num.end(); it++) {
      ...
      minimum->SetVariable(4,"#sigma_{K}",   0.2,  0.001);
      // do the minimization
      minimum->Minimize();
