...
int compare_each_run_xbj(){
  ...
  for(json::iterator it = j_Q2z.begin();it!=j_Q2z.end();++it){
    ...
    for(json::iterator it  = j_zx.begin();it!=j_zx.end();++it){
      ...
      auto j_x = it.value();
      std::string canvas_name = "Q2_z_"+std::to_string(Q2).substr(0,5)+"_"+std::to_string(z).substr(0,4);
      std::string q2x_name = "Q2_z"+std::to_string(Q2).substr(0,5)+"_"+std::to_string(z).substr(0,4)+"_yieldratio";
      TCanvas *c_Q2z_ratio = new TCanvas("",q2x_name.c_str(),1900,1000);
      TH1D* h_neg_q2z = new TH1D("",(q2x_name).c_str(),100,0,1);
      TH1D* h_pos_q2z = new TH1D("",(q2x_name).c_str(),100,0,1);
      ...
      if(z !=0 && Q2!=0){  
        for(json::iterator it = j_x.begin();it!=j_x.end();++it){
          ...
          TH1D *h_xbj_neg_sim = new TH1D("","neg sim sum",100,0,1);
          TH1D *h_xbj_neg_sim_incrad = new TH1D("","neg sim inc rad",100,0,1);
          h_xbj_neg_sim_incrad = (TH1D*)rootfile_neg_sim->Get("xbj_neg_inc_rad");
          TH1D *h_xbj_neg_sim_excrad = new TH1D("","neg sim exc rad",100,0,1);
          h_xbj_neg_sim_excrad = (TH1D*)rootfile_neg_sim->Get("xbj_neg_exc_rad");
          TH1D *h_xbj_neg_sim_rho = new TH1D("","neg sim rho",100,0,1);
          h_xbj_neg_sim_rho = (TH1D*)rootfile_neg_sim->Get("xbj_neg_rho");
          TH1D *h_xbj_neg_sim_delta = new TH1D("","neg sim delta",100,0,1);
          ...
          TH1D *h_xbj_pos_sim = new TH1D("","pos sim sum",100,0,1);
          TH1D *h_xbj_pos_sim_incrad = new TH1D("","pos sim inc rad",100,0,1);
          h_xbj_pos_sim_incrad = (TH1D*)rootfile_pos_sim->Get("xbj_pos_inc_rad");
          TH1D *h_xbj_pos_sim_excrad = new TH1D("","pos sim exc rad",100,0,1);
          h_xbj_pos_sim_excrad = (TH1D*)rootfile_pos_sim->Get("xbj_pos_exc_rad");
          TH1D *h_xbj_pos_sim_rho = new TH1D("","pos sim rho",100,0,1);
          h_xbj_pos_sim_rho = (TH1D*)rootfile_pos_sim->Get("xbj_pos_rho");
          TH1D *h_xbj_pos_sim_delta = new TH1D("","pos sim delta",100,0,1);
          ...
          if(xbj!=0){
            ...
            for(auto it = neg_D2_runs.begin();it!=neg_D2_runs.end();++it){
              ...
              TGraph* g_yield_negsim = new TGraph(h_xbj_neg_sim);
              g_yield_negsim->SetTitle("simc");
              mg_neg->Add(g_yield_negsim,"L");

            TCanvas* c_yield_neg = new TCanvas();
            mg_neg->GetXaxis()->SetTitle("xbj");
            mg_neg->GetYaxis()->SetTitle("yield");
            mg_neg->Draw("A");
            c_yield_neg->BuildLegend(0.85,0.65,0.95,0.95);
            std::string c_yield_neg_name = "results/yield/check_xbj/yieldcheck_"+std::to_string(RunGroup)+"_neg.pdf";
            ...
            for(auto it = pos_D2_runs.begin();it!=pos_D2_runs.end();++it){
              ...
              TGraph* g_yield_possim = new TGraph(h_xbj_pos_sim);
            ...
            std::string c_yield_pos_name = "results/yield/check_xbj/yieldcheck_"+std::to_string(RunGroup)+"_pos.pdf";
