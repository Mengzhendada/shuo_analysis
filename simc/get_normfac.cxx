...
void get_normfac(int RunGroup = 0){
  ...
  std::string hist_D2_neg_exc_norad_name = "/group/c-csv/shuo/simc_gfortran/outfiles/csv_"+std::to_string(RunGroup)+"_D2_neg_exc_norad.hist";
  ...
  line = "";
  while(std::getline(D2_neg_exc_norad,line)){
    if(line.find(normfac)<line.length()){
      std::cout<<line<<std::endl;
      size_t i = 0;
      for(;i < line.length();i++){ if(isdigit(line[i])) break; }
  ...
  line = "";
  while(std::getline(D2_pos_exc_norad,line)){
    if(line.find(normfac)<line.length()){
      std::cout<<line<<std::endl;
      size_t i = 0;
      for(;i < line.length();i++){ if(isdigit(line[i])) break; }
