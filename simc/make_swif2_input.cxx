...
int make_swif2_input(){
  json j_input;
  int cpuCores = 1;
  long int diskBytes = 20000000000;
  std::string name = "csv";
  std::string os = "centos79";
  ...
  for(int i = 1;i<59;i++){
    ...
    json j_each;
    j_each["account"] = "hallc";
    j_each["command"].push_back(command_new);
    j_each["constraint"] = os;
    j_each["cpu_cores"] = cpuCores;
    j_each["disk_bytes"] = diskBytes;
    j_each["name"] = name;
    j_each["partition"] = "production";
    j_each["ram_bytes"] = ramBytes;
    j_each["stderr"] = errout;
    j_each["stdout"] = outout;
    j_each["time_secs"] = timeSecs;

    j_jobs.push_back(j_each); 
  }
  j_input["jobs"] = j_jobs;
  std::ofstream ofs("shuo_analysis/simc/csv_simc_2.json");
