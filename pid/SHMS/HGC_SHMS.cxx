...
//  c_proton_p->SaveAs("results/hgc_costh_proton_p.pdf");
  ...
  TLine *l_low = new TLine(0.9*SHMS_P,0,0.9*SHMS_P,0.005);
  TLine *l_high = new TLine(1.22*SHMS_P,0,1.22*SHMS_P,0.005);
  TLine *l_center = new TLine(SHMS_P,0,SHMS_P,0.005);
  TCanvas *c_all = new TCanvas();
  f_pi_2->SetLineColor(kRed);
  f_pi_2->Draw();
  f_pi_2->GetYaxis()->SetRangeUser(0,0.005);
  f_pi_2->GetYaxis()->SetTitle("hgc npe/N0");
  ...
  l_high->Draw("same");
  l_center->Draw("same");
  std::string c_all_name = "results/pid/rftime/hgc_npe_"+std::to_string(RunGroup)+".pdf";
