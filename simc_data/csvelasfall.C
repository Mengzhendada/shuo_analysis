...
void csvelasfall(){
  ...
  json j;
  {
    std::ifstream json_input_file("elasrunlist1.json");
    json_input_file >> j;
  }
  ...
  /*for(auto& line: data){
    ...
    std::cout<<'\n';}
    */
  TH2F *h_yield_beforecut = new TH2F("yield data before cut", "yield data", 10,
                                     6008, 6018, 100, 0, 200);
  TH2F *h_yield_aftercut = new TH2F("yield data after cut", "yield data", 10,
                                    6008, 6018, 100, 0, 100);
  TH2F *h_yield_sim =
      new TH2F("yield sim", "yield sim", 10, 6008, 6018, 100, 0, 200);
  // TH2F *h_yield_sim2 = new TH2F("yield sim","yield
  // sim",10,6008,6018,100,0,200);
  ...
  for (json::iterator it = j.begin(); it != j.end(); ++it) {
    ...
    std::string rootfile_data = fmt::format("/group/c-csv/shuo/replay/ROOTfiles/coin_replay_production_{}_-1.root", runnumber);
