...
void elas_compare(){
  ...
  for(auto it = j_simc.begin();it!=j_simc.end();++it){
    ...
    //}
      ...
      auto d_data_1 = d_data.Filter("P.gtr.dp>-10 && P.gtr.dp<-2");
      std::cout<<"RunNumber "<<RunNumber<<" delta 1 counts "<<*d_data_1.Count()<<std::endl;
      auto d_data_2 = d_data.Filter("P.gtr.dp>-6 && P.gtr.dp<-2");
      auto d_data_3 = d_data.Filter("P.gtr.dp>-2 && P.gtr.dp<2");
      auto d_data_4 = d_data.Filter("P.gtr.dp>2 && P.gtr.dp<6");
      auto d_data_5 = d_data.Filter("P.gtr.dp>2 && P.gtr.dp<10");
      std::cout<<"RunNumber "<<RunNumber<<" delta 5 counts "<<*d_data_5.Count()<<std::endl;
      if(*d_data_1.Count()>5 && *d_data_5.Count()>5){
      auto W_1 = d_data_1.Histo1D({"","1",10,0.4,1.2},"W");
      ...
      auto W_5 = d_data_5.Histo1D({"","5",10,0.4,1.2},"W");
      ...
      c_radia->cd(2);
      TVirtualPad* Pad_sim = c_radia->cd(1);
      Pad_sim->SetLogz();
      Em_Pm_sim->SetBit(TH1::kNoStats);
      Em_Pm_sim->DrawCopy("LEGO");
      std::string c_radia_name = "results/simc_data/EM_PM"+std::to_string(RunNumber)+".pdf";
