...
void make_latex_xbj(){
  ...
  for(json::iterator it = j_Q2z.begin();it!=j_Q2z.end();++it){
     ...
     for(json::iterator i_z = j_zx.begin();i_z!=j_zx.end();++i_z){
      ...
      if(counts_x_name.size()>2){
      ofs<<"\\begin{frame}{TE,pi eff, pi purity corrected yield}"<<std::endl;
      ofs<<"\\begin{columns}"<<std::endl;
      ofs<<"\\begin{column}[T]{0.5\\textwidth}"<<std::endl;
      ofs<<"\\includegraphics[width = 0.7\\textwidth]{results/yield/check_xbj/yieldcheck_"<<rungroups[2]<<"_neg.pdf}"<<std::endl;

      ofs<<"\\end{column}"<<std::endl;
      ofs<<"\\begin{column}[T]{0.5\\textwidth}"<<std::endl;
      ofs<<"\\includegraphics[width = 0.7\\textwidth]{results/yield/check_xbj/yieldcheck_"<<rungroups[2]<<"_pos.pdf}"<<std::endl;
      ofs<<"\\end{column}"<<std::endl;
      ofs<<"\\end{columns}"<<std::endl;

      if(counts_x_name.size()>3){
      ofs<<"\\begin{columns}"<<std::endl;
      ofs<<"\\begin{column}[T]{0.5\\textwidth}"<<std::endl;
      ofs<<"\\includegraphics[width = 0.7\\textwidth]{results/yield/check_xbj/yieldcheck_"<<rungroups[3]<<"_neg.pdf}"<<std::endl;
      ofs<<"\\end{column}"<<std::endl;
      ofs<<"\\begin{column}[T]{0.5\\textwidth}"<<std::endl;
      ofs<<"\\includegraphics[width = 0.7\\textwidth]{results/yield/check_xbj/yieldcheck_"<<rungroups[3]<<"_pos.pdf}"<<std::endl;
      ofs<<"\\end{column}"<<std::endl;
      ofs<<"\\end{columns}"<<std::endl;
      }
      ofs<<"\\end{frame}"<<std::endl;
      ...
      ofs<<"\\begin{columns}"<<std::endl;
      ofs<<"\\begin{column}[T]{0.5\\textwidth}"<<std::endl;
      ofs<<"\\includegraphics[width = 0.9\\textwidth]{results/yield/statistics_q2zcorr/"<<counts_x_name[2]<<"_ratio.pdf}"<<std::endl;

      ofs<<"\\end{column}"<<std::endl;
      ofs<<"\\begin{column}[T]{0.5\\textwidth}"<<std::endl;
      if(counts_x_name.size()>3){
      ofs<<"\\includegraphics[width = 0.9\\textwidth]{results/yield/statistics_q2zcorr/"<<counts_x_name[3]<<"_ratio.pdf}"<<std::endl;
      }
      ofs<<"\\end{column}"<<std::endl;
      ofs<<"\\end{columns}"<<std::endl;
      }
