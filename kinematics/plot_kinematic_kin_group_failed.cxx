...
void plot_kinematic_kin_group(){
  ...
  //  std::string color[13] = {}
  int i = 0;
  auto *c_neg = new TCanvas("csv kinematics pi-","csv pi-",2200,1450);
  //auto *c_pos = new TCanvas("csv kinematics pi+","csv pi+",2200,1450);
  //c->Divide(2,1);
  //c->cd(1);
  ...
  TFile* kinematics_root = new TFile("results/csv_kin/kin_plot/kinematics.root","RECREATE");
  for(json::iterator it = j.begin();it!=j.end();++it)
  {
    ...
    else
    {
      for(json::iterator ik = runjs.begin();ik!=runjs.end();++ik)
      {
        ...
        TH2F *h_pos_all = new TH2F(name_pos.c_str(),name_pos.c_str(),400,0,1,400,0,10);
        h_pos_all->GetXaxis()->SetTitle("x");
        h_pos_all->GetYaxis()->SetTitle("Q2");
        TH2F *h_neg_all = new TH2F(name_neg.c_str(),name_neg.c_str(),400,0,1,400,0,10);
  ...
  c_neg->BuildLegend();
  c_neg->SaveAs("results/csv_kin/kin_plot/kinematics_neg.png");
}
